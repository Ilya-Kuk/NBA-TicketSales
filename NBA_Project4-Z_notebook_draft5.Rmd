...
# Since there are so many zeroes, this may skew the regression models. This removes all zero-values for regression response variables and fits new trees.
...
```{r}
dataset.ptt <- dataset.ptt[-post_total_tickets==0,]
```
...
A_methods <- A_methods[order(A_methods[,2],decreasing = FALSE),]
A_methods
Top <- A_methods[1,]
```
```{r,echo=FALSE}
...
```{r}
dataset.ptta <- dataset.ptt[-post_total_ticket_amt==0,]
```
...
```{r}
dataset.patp <- dataset.patp[-post_avg_ticket_price==0,]
```
