...
Pruning the Tree
```{r}
cv.ptta_train <- cv.tree(tree.ptta_train, FUN=prune.misclass)
cv.ptta_train
...
prune.ptta_train <- prune.misclass(tree.ptta_train, best=11)
...
boost.ptta_train <- gbm(post_total_ticket_amt~., data=dataset.pttaB[train,], distribution='gaussian', n.trees=5000, interaction.depth=4)
boost.ptta_pred <- predict(boost.ptta_train, dataset.pttaB[-train,], n.trees=5000)
