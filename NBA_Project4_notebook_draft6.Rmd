...
```{r}
d_train <- read.csv("nbasample2GrpPlus0505.csv",header=T, sep=",")
d_test <- read.csv("validationSample1k0506.csv",header=T, sep=",")
```
...
### Removing other response variables besides post_ticket_response

```{r}
#Finding position of response variable
Q <- match('post_ticket_response',a) #new response variable in this dataset
a[Q]
...
```{r}
#Growing Tree
tree.ptf <- tree(post_ticket_response~., dataset.ptf)
#Plotting Tree
plot(tree.ptf)
...
```{r}
#Growing Tree
tree.ptf_train <- tree(post_ticket_response~., dataset.ptf, subset=train)
#Plotting Tree
plot(tree.ptf_train)
...
#Confusion Matrix
M <- table(tree.ptf_pred, post_ticket_response[-train])
M
...
#Confusion Matrix
M_prune <- table(prune.ptf_pred, post_ticket_response[-train])
M_prune
...
```{r}
#Growing Tree
bag.ptf_train <- randomForest(post_ticket_response~., data=dataset.ptf0, subset=train, mtry=ncol(dataset.ptf0)-1, ntrees=1000, importance=TRUE)
#Can't plot tree
#Fitting tree on test set
bag.ptf_pred <- predict(bag.ptf_train, dataset.ptf0[-train,], type="class")
#Confusion Matrix
M_bag <- table(bag.ptf_pred, post_ticket_response[-train])
M_bag
...
```{r}
#Growing Tree
RF.ptf_train <- randomForest(post_ticket_response~., data=dataset.ptf0, subset=train, ntrees=1000, importance=TRUE)
#Can't plot tree
```
...
#Confusion Matrix
M_RF <- table(RF.ptf_pred, post_ticket_response[-train])
M_RF
...
dataset.ptfB$post_ticket_response <- y
#distribution='bernoulli' requires 0,1 variable
#Growing Tree
boost.ptf_train <- gbm(post_ticket_response~., data=dataset.ptfB[train,], distribution='bernoulli', n.trees=1000, interaction.depth=4)
#Can't plot tree
```
...
#Confusion Matrix
M_boost <- table(boost.ptf_pred, post_ticket_response[-train])
...
MSE.ptt
A_methods <- rbind(A_methods, c('Tree',MSE.ptt,sqrt(MSE.ptt)))
```
```{r,echo=FALSE}
...
MSE.prune.ptt
A_methods <- rbind(A_methods, c('Cross Validated Tree',MSE.prune.ptt,sqrt(MSE.prune.ptt)))
```
```{r,echo=FALSE}
...
MSE.bag.ptt
A_methods <- rbind(A_methods, c('Bagged Tree',MSE.bag.ptt,sqrt(MSE.bag.ptt)))
```
```{r,echo=FALSE}
...
MSE.RF.ptt
A_methods <- rbind(A_methods, c('Random Forest Tree',MSE.RF.ptt,sqrt(MSE.RF.ptt)))
```
```{r,echo=FALSE}
...
MSE.boost.ptt
A_methods <- rbind(A_methods, c('Boosted Tree',MSE.boost.ptt,sqrt(MSE.boost.ptt)))
```
```{r,echo=FALSE}
...
```{r}
A_methods <- A_methods[-1,]
A_methods <- A_methods[order(A_methods[,3],decreasing = FALSE),]
A_methods
Top <- A_methods[1,]
```
```{r,echo=FALSE}
print(paste("The most effective method was the",Top[1],", with",Top[3],"Square root MSE."))
A_methods <- matrix(c("","",""),
                    nrow=1,
                    ncol=3)
colnames(A_methods) <- c("Method","Mean Squared Error","Square Root MSE")
...
MSE.ptta
A_methods <- rbind(A_methods, c("Tree",MSE.ptta,sqrt(MSE.ptta)))
```
```{r,echo=FALSE}
...
MSE.prune.ptta
A_methods <- rbind(A_methods, c("Cross Validated Tree",MSE.prune.ptta,sqrt(MSE.prune.ptta)))
```
```{r,echo=FALSE}
...
MSE.bag.ptta
A_methods <- rbind(A_methods, c("Bagged Tree",MSE.bag.ptta,sqrt(MSE.bag.ptta)))
```
```{r,echo=FALSE}
...
MSE.RF.ptta
A_methods <- rbind(A_methods, c("Random Forest Tree",MSE.RF.ptta,sqrt(MSE.RF.ptta)))
```
```{r,echo=FALSE}
...
MSE.boost.ptta
A_methods <- rbind(A_methods, c("Boosted Tree",MSE.boost.ptta,sqrt(MSE.boost.ptta)))
```
```{r,echo=FALSE}
...
MSE.patp
A_methods <- rbind(A_methods, c("Tree",MSE.patp,sqrt(MSE.patp)))
```
```{r,echo=FALSE}
...
MSE.prune.patp
A_methods <- rbind(A_methods, c("Cross Validated Tree",MSE.prune.patp,sqrt(MSE.prune.patp)))
```
```{r,echo=FALSE}
...
MSE.bag.patp
A_methods <- rbind(A_methods, c("Bagged Tree",MSE.bag.patp,sqrt(MSE.bag.patp)))
```
```{r,echo=FALSE}
...
MSE.RF.patp
A_methods <- rbind(A_methods, c("Random Forest Tree",MSE.RF.patp,sqrt(MSE.RF.patp)))
```
```{r,echo=FALSE}
...
MSE.boost.patp
A_methods <- rbind(A_methods, c("Boosted Tree",MSE.boost.patp,sqrt(MSE.boost.patp)))
```
```{r,echo=FALSE}
