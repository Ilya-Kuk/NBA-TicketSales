...
rm(d_train, d_test) #cleaning up environment
```
...
rm(V_fact,I_fact,L_fact,L_temp,x) #clearing environment space
```
...
a <- colnames(d)
rm(TEAM, TEAMN) #clearing environment space
```
```{r,results='hide',message=FALSE}
attach(d)
```
...
#POSTN
a[POSTN]
rm(POST, POSTN) #clearing environment space
```
...
A_methods <- rbind(A_methods, c('log',(1-(M_glm[1,2]+M_glm[2,1])/(sum(M_glm)))))
#ROC Curves
rocplot_p(glm.prr_probs, na.omit(d)$post_retail_response) #must na.omit transform the data first - only 3271 entries remaining
rocplot_n(glm.prr_probs, na.omit(d)$post_retail_response)
...
A_methods <- rbind(A_methods, c('log1',(1-(M_glm1[1,2]+M_glm1[2,1])/(sum(M_glm1)))))
#ROC Curves
##certain variables with NA's removed, using omitted dataset to make predictions
glm.prr1_ROC <- glm(post_retail_response ~ days_since_most_recent_email + days_since_most_recent_click + tot_emails_30_days_prior + email_domain_group + ind_nbastore_90d + pre_regular_season_tix + pre_tickets_ever + fan_mths_since_last_purch + fan_purch_web_ind + lp_purch_mc_ind + emailopendays + pre_total_tickets_cat + tot_opens_90dprior_cat + mths_since_purch_cat + fancardcat, data=na.omit(dataset.prr), family=binomial) 
glm.prr1_probs_ROC <- predict(glm.prr1_ROC,type='response')
rocplot_p(glm.prr1_probs_ROC, na.omit(d)$post_retail_response) #must na.omit transform the data first - only 3271 entries remaining
rocplot_n(glm.prr1_probs_ROC, na.omit(d)$post_retail_response)
#Attempting to improve
...
A_methods <- rbind(A_methods, c('log2',(1-(M_glm2[1,2]+M_glm2[2,1])/(sum(M_glm2)))))
#ROC Curves
##certain variables with NA's removed, using omitted dataset to make predictions
glm.prr2_ROC <- glm(post_retail_response ~ days_since_most_recent_email + days_since_most_recent_click + tot_emails_30_days_prior + email_domain_group + ind_nbastore_90d + pre_regular_season_tix + pre_tickets_ever + fan_mths_since_last_purch + fan_purch_web_ind + lp_purch_mc_ind + emailopendays + pre_total_tickets_cat + tot_opens_90dprior_cat + mths_since_purch_cat + fancardcat, data=na.omit(dataset.prr), family=binomial) 
glm.prr2_probs_ROC <- predict(glm.prr2_ROC,type='response')
rocplot_p(glm.prr2_probs_ROC, na.omit(d)$post_retail_response) #must na.omit transform the data first - only 3271 entries remaining
rocplot_n(glm.prr2_probs_ROC, na.omit(d)$post_retail_response)
```
